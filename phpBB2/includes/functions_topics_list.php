...
//--------------------------------------------------
function topic_list($box, $tpl='', $topic_rowset=array(), $list_title='', $split_type=false, $display_nav_tree=true, $footer='', $inbox=true, $select_field='', $select_type=0, $select_formname='', $select_values=array())
{
	...
	for ($i=0; $i < count($topic_rowset); $i++)
	{
		...
		else
		{
			...
			else if( $topic_rowset[$i]['topic_type'] == POST_NEWS )
			{
...
//-- end mod : today at   yesterday at ------------------------------------------------------------------------ 
				...
				{
					$last_post_author = '<a href="' . append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . '='  . $topic_rowset[$i]['id2']) . '">'.color_group_colorize_name($topic_rowset[$i]['id2']).'</a>';
				}
				else
				{
...
// END cmx_mod 
		...
		$this_auth2 = (!$cat_hierarchy) ? $is_auth[$forum_id] : $this_auth[$forum_id];
		$can_edit = ($this_auth2['auth_mod'] || (($topic_rowset[$i]['user_id'] == $userdata['user_id']) && ($topic_rowset[$i]['topic_status'] != TOPIC_LOCKED) && $this_auth2['auth_edit'])) ? true : false;
		...
		if ($can_edit)
		{
			$template->assign_block_vars($tpl . '.row.topic.can_edit_title', array());
		}
