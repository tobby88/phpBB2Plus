...
function phpbb_style_import_validate_batch($db, $batch, $header)
{
	$names = array();
...
function phpbb_style_import_register_batch($db, $batch, $header, $default)
{
	$default_id = null;
	$ids = array();
...
function phpbb_style_import($database, $request, $header, $entries, $archive, $publish)
{
	...
	try
	{
		for ($phase = 0; ; $phase++)
		{
			...
			phpbb_style_import_require_available($db, array($header['template']));
			phpbb_style_import_validate_batch($db, $batch, $header);
		...
		$default_id = phpbb_style_import_register_batch($db, $batch, $header, $default);
