...
// : If the we are checking a personal gallery and it doesn't exists
...
function album_permissions($user_id, $cat_id, $permission_checks, $catdata = 0)
{
	...
	$is_admin = $logged_in && $userdata['user_level'] == ADMIN;
	...
	else
	{
	 	...
	 	// ------------------------------------------------------------------------
		if ($thiscat['cat_user_id'] != 0 && $thiscat['cat_id'] == 0)
		{
		 	$album_permission = personal_gallery_access(1,1); //$view_check, $upload_check);
		}
	}
	...
	// ------------------------------------------------------------------------
	// if we set our $thiscat and not our permission array then we must
	// authenticate it
	// ------------------------------------------------------------------------
	if (album_is_debug_enabled() == true)
	{
		album_debug('album_permissions : before album_user_access : %s(id=%d), $album_permission = %s',$thiscat['cat_title'],$thiscat['cat_id'], $album_permission);
	}

    if (!empty($thiscat)  && !is_array($album_permission))
	{
    	$album_permission = album_user_access($cat_id, $thiscat, $view_check, $upload_check, $rate_check, $comment_check, $edit_check, $delete_check);
	}

	if (album_is_debug_enabled() == true)
	{
		album_debug('album_permissions : after album_user_access : %s(id=%d), $album_permission = %s',$thiscat['cat_title'],$thiscat['cat_id'], $album_permission);
	}
	// ------------------------------------------------------------------------
	...
	// Public/guest sentinel IDs are not personal-gallery owners. Possession of
	// an old user ID without a logged-in session must not grant ownership either.
	$is_owner = $logged_in && isset($thiscat['cat_user_id']) && (int) $thiscat['cat_user_id'] > 0
		&& (int) $thiscat['cat_user_id'] === (int) $userdata['user_id'];
	...
	if ($thiscat['cat_user_id'] != 0)
	{
		...
		switch ($album_config['personal_gallery'])
		{
			case ALBUM_USER:
				...
				// ------------------------------------------------------------------------
                if (empty($thiscat) || $thiscat['cat_id'] == 0 || $cat_id == ALBUM_ROOT_CATEGORY)
    			{
					// ------------------------------------------------------------------------
        ...
        // ------------------------------------------------------------------------
     	switch ($thiscat['cat_upload_level'])
     	{
     	 	case ALBUM_PRIVATE:
     	 		// ------------------------------------------------------------------------
     	 		...
     	 		// if ($thiscat['cat_id'] != 0 && ($user_id == $userdata['user_id']) )
     	 		// ------------------------------------------------------------------------
				if ( $thiscat['cat_id'] != 0 && $is_owner )
     	 		{
					if ($album_config['personal_gallery'] == ALBUM_ADMIN && !$is_admin)
     	 		 	{
     	 		 	 	$album_permission['upload'] = 0;
					}
     	 		 	else
		...
		// ------------------------------------------------------------------------
		if ( $is_admin ||
			 ($album_permission['upload'] == 1 && $album_config['personal_allow_gallery_mod'] == 1 &&
			  $is_owner) )
		{
