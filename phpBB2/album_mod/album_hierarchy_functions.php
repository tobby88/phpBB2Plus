...
// $album_data  : structur description
// indexes :
...
function album_display_admin_index($cur = ALBUM_ROOT_CATEGORY, $level = 0, $max_level = -1, $column_offset=1)
{
    ...
    // display 'the' level
    $AC_this = isset($album_data['keys'][$cur]) ? $album_data['keys'][$cur] : ALBUM_ROOT_CATEGORY; //-1;
	...
	else
	{
		...
		$is_root = (($AC_this == ALBUM_ROOT_CATEGORY || $AC_this == 0)) ? true : false;
	}

    // root level
    if ($AC_this == ALBUM_ROOT_CATEGORY)
	{
...
function album_build_index($user_id, &$keys, $cur_cat_id = ALBUM_ROOT_CATEGORY, $real_level = ALBUM_ROOT_CATEGORY, $max_level = ALBUM_ROOT_CATEGORY, $newestpic = NULL)
{
    ...
    $album_show_pic_url = ( (defined('ALBUM_SP_CONFIG_TABLE')) ? "album_showpage.$phpEx" : "album_page.$phpEx");

    // display the level
    $AC_this = isset($album_data['keys'][$cur_cat_id]) ? $album_data['keys'][$cur_cat_id] : ALBUM_ROOT_CATEGORY;
	// root level head
	...
	if ( $level == ALBUM_ROOT_CATEGORY + 1 )
	{
        ...
        for ($j = 0; $j < count($current_subcategories); $j++)
		{
            ...
            if ($subname != '')
            {
                $link = '<a href="' . $subpgm . '" title="' . $subdesc . '" class="gensmall">' . $subname . '</a>' . $sub_total_pics;
			}
            ...
            if ($link != '')
			{
        		...
        		if ( $total > 0 )
        		{
                    $new_text = ($total > 1) ? sprintf($lang['Multiple_new_pictures'], $total) : sprintf($lang['One_new_picture'], $total);
					$newpics_sub_link = ' <img src="'.$images['mini_new_pictures'].'" alt="' . $new_text .'" title="' . $new_text .'">';
        		 	$link = $link . $newpics_sub_link;
				}
        ...
        if ( !empty($links) )
        {
	        if ( ($user_id==ALBUM_PUBLIC_GALLERY && $album_config['show_index_subcats']==1) ||
	        	 ($user_id!=ALBUM_PUBLIC_GALLERY && $album_config['personal_show_subcats_in_index']==1) )
			{
...
function album_build_tree(&$cats, &$parents, $level = ALBUM_ROOT_CATEGORY, $parent = ALBUM_ROOT_CATEGORY) {
	...
	{
        $AC_this = count($album_data['data']);
        $key = $album_data_level['id'][$i];
        $album_data['sub'][$parent][] = $key;
        $album_data['keys'][$key]     = $AC_this;
...
function album_get_object_lang($cur_cat_id, $field) 
{
    ...
    $res = '';
    $AC_this = $album_data['keys'][$cur_cat_id];
...
function album_make_nav_tree($cur_cat_id, $pgm, $nav_class = 'nav', $user_id = ALBUM_PUBLIC_GALLERY)
{
    ...
    while (($AC_this >= 0) || ($fcur != ''))
	{
        $field_name = album_get_object_lang($cur_cat_id, 'name');
        $param_type = 'cat_id';
        $param_value = $album_data['id'][$AC_this];
		...
		{
            $cur_cat_id = $album_data['parent'][$AC_this];
        }
...
function album_build_jumpbox($cat_id, $user_id = ALBUM_PUBLIC_GALLERY, $auth_key = ALBUM_AUTH_VIEW)
{
	...
	$res .= '<form name="jumpbox" action="'. append_sid("album_cat.$phpEx") .'" method="get">';
...
{
    global $lang, $board_config, $template, $album_data, $phpEx, $userdata;
...
function album_generate_index_columns()
{
	...
	album_generate_super_cells($header_col_span-1, $indexes);
