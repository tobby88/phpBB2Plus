	...
	function handle_attachments($mode)
	{
		...
		for ($i = 0; $i < sizeof($actual_list); $i++)
		{
			if ($actual_id_list[$i] != 0)
			{
				...
				{
					message_die(GENERAL_ERROR, 'You tried to change an attachment you do not have access to', '');
				}
			}
			else
			{
	...
	*/
	function display_attachment_bodies()
	{
		...
		$template->assign_vars(array(
			...
			'RULES'							=> '<a href="' . append_sid($phpbb_root_path . "attach_rules.$phpEx?f=$u_rules_id") . '" target="_blank" rel="noopener noreferrer">' . $lang['Allowed_extensions_and_sizes'] . '</a>',
			...
			if (!$error) 
			{
				...
				if (!$cryptic)
				{
					...
					$this->attach_filename = preg_replace_callback(
						...
						function ($match)
						{
							return chr((ord($match[1]) << 6 & 0xC0) | (ord($match[2]) & 0x3F));
						},
