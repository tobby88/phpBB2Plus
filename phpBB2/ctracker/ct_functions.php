...
if ( !defined('IN_PHPBB') )
{
  ...
  function ct_filllog()
  {
    ...
    $ctl_pmeld    = 1;
    $ctl_stamp    = time();
    $ctl_remotead = $HTTP_SERVER_VARS['REMOTE_ADDR'] . " - (" . $userdata['username'] . ")";
    ...
    {
      $clog = @fopen($phpbb_root_path . "ctracker/logs/logfile_flood.txt", "a") or die("Could not open Logfile.");
      @ftruncate($clog, '0');
      ...
      $cfp = @fopen ($countername, 'a') or die("Could not open Counterfile.");
      @ftruncate($cfp, '0');
