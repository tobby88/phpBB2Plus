...
require('./pagestart.' . $phpEx);
  ...
  if($seite == 1 || $seite == 2 || $seite == 3)
  {
    ...
    for ($i = count($filename)-1; $i >= 0; $i--)
    {
      ...
      if(htmlspecialchars($line[0]) == 0)
      {
        $row_class = 'row3';
        $line[2] = '>> ' . htmlspecialchars($line[2]) . ' <<';
        $line[3] = '>> ' . htmlspecialchars($line[3]) . ' <<';
        $line[4] = '>> ' . htmlspecialchars($line[4]) . ' <<';
        $line[5] = '>> ' . htmlspecialchars($line[5]) . ' <<';
      }

    $template->assign_block_vars('ctrack_logoutput', array(
      'L_DATE_CELL'   => date('d.m.Y, H:i', htmlspecialchars($line[1])),
      'L_C1'          => htmlspecialchars($line[2]),
      'L_C2'          => htmlspecialchars($line[3]),
      'L_C3'          => htmlspecialchars($line[4]),
      'L_C4'          => htmlspecialchars($line[5]),
