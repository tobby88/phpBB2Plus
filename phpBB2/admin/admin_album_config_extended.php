...
// the language files....
require_once($phpbb_root_path . 'language/lang_' . $board_config['default_lang'] . '/lang_main_album.' . $phpEx);
require_once($phpbb_root_path . 'language/lang_' . $board_config['default_lang'] . '/lang_admin_album.' . $phpEx);
...
for ($outer = 0; $outer < count($album_config_tabs); $outer++)
{
	$template->assign_block_vars('header_row', array(
			...
			'HEADER_TAB_CLASS' => (strcasecmp($selected_tab,$album_config_tabs[$outer]['selection']) != 0) ? 'row2' : 'tab_headers',
			'TAB_LINKS' => (strcasecmp($selected_tab,$album_config_tabs[$outer]['selection']) != 0) ? '' : 'tab_links'
		)
	);
