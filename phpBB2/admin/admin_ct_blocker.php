...
$template->set_filenames(array(
  ...
  if(!empty($_GET['ctrun']))
  {
	...
	@unlink($phpbb_root_path . 'cache/ct_proxylist.'.$phpEx);
  }

  if(isset($HTTP_POST_VARS['submit']) && !empty($HTTP_POST_VARS['entry']))
  {
    $neueintrag = '';
    $neueintrag = htmlspecialchars(addslashes(str_replace("\'", "''", $HTTP_POST_VARS['entry'])));
  ...
  {
    $dellist = $row['list'];
    $dellink = append_sid("admin_ct_blocker." . $phpEx . "?ctrun=" . $row['id']);
