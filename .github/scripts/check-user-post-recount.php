...
class RecountConnection {
 ...
 function sql_query($sql){
  ...
  try{$r=$this->pdo->query($sql);$this->affected=$r->rowCount();return preg_match('/^SELECT/',$sql)?new RecountRows($r->fetchAll(PDO::FETCH_ASSOC)):true;}catch(PDOException $e){return false;}
 }
...
try{
 foreach($native?array('MyISAM','InnoDB'):array('SQLite') as $engine){foreach(array('english','german') as $locale){
  ...
  foreach(array('success','failure','invalid','skipped') as $case){
   ...
   recount_check($board_locks===($case==='invalid'?array():array(array(false,true,false),array(true,true,false))),'Actual controller restores maintenance state');
