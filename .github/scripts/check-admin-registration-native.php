...
function qad_suite()
{
 ...
 foreach(array('users','sessions','jr','config','groups','user_group','disallow','words','banlist','themes') as $s){qad_reset();rgn_sql('ALTER TABLE fixture_'.$s.' ENGINE=MyISAM');$before=rgn_snap();ats_check(!qad_success(qad_run())&&rgn_snap()===$before,'Reject nontransactional participant '.$s);rgn_sql('ALTER TABLE fixture_'.$s.' ENGINE=InnoDB ROW_FORMAT=DYNAMIC');$cases++;}
 ...
 foreach(array('users','sessions','jr_admin_users','config','groups','user_group','disallow','words','banlist','themes') as $s){ats_check(in_array('fixture_'.$s,$covered,true),'Existing migration covers ACP participant '.$s);}
