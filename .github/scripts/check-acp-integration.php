...
$integrations = array(
	...
	'admin/admin_board.php' => array('cookie_consent_enable', 'sfs_enable', 'phpbb_admin_require_post_session();', 'phpbb_admin_session_field()', 'phpbb_board_config_save($db, $_POST)'),
	'admin/admin_board_extend.php' => array("\$module['General']['Configuration_extend']", 'phpbb_admin_require_post_session();', 'phpbb_admin_session_field()', 'phpbb_mod_settings_save($db, $_POST,', '$dir !== false'),
	'includes/functions_mod_settings_storage.php' => array('$db->sql_escape($value)', '$db->sql_escape($key)', '$actor[\'guard\']', "'START TRANSACTION'", "'COMMIT'"),
	...
	'includes/functions_jr_admin.php' => array('$module = array();', '$module_list = array();', 'if ($dir === false)', '$language_dir = $phpbb_root_path', 'phpbb_profile_text(isset($jr_admin_userdata[\'admin_notes\'])', '$user_id = max(0, intval($user_id));', 'basename((string) $file)', "preg_match('/^[a-f0-9]{32}$/D'", 'hash_equals((string) $userdata[\'session_id\']', 'function jr_admin_prepare_navigation_modules', "'Extreme_Styles' => 'Styles'", "'Extensions' => 'Attachments'", "'Custom_Profile' => 'Users'", "'Systeminfo' => 'General'", 'return jr_admin_prepare_navigation_modules($module_list);'),
	...
	'admin/admin_groups.php' => array("\$module['Groups']['Manage']", 'phpbb_admin_require_post_session();', 'phpbb_admin_session_field()', 'phpbb_group_admin_save($db, $_POST)', 'phpbb_group_admin_actor(new PhpbbGroupDatabase($db))'),
