...
$storage = file_get_contents($root . '/phpBB2/includes/functions_group_storage.php');
...
group_control_test_assert(strpos($storage, 'group_policy.group_type = ') !== false && strpos($storage, 'GROUP_OPEN') !== false, 'subscriptions must target a currently open group');
group_control_test_assert(strpos($storage, 'AND NOT EXISTS (SELECT 1 FROM ') !== false, 'subscriptions must avoid duplicate membership rows');
group_control_test_assert(strpos($storage, 'if (!$applied) { continue; }') !== false, 'unchanged memberships must not generate mail recipients');
group_control_test_assert(strpos($storage, '$db->sql_escape($name)') !== false, 'moderator-entered usernames must use database-driver escaping');
group_control_test_assert(strpos($groupcp, 'phpbb_group_change($db, $action, $original_group, $value)') !== false, 'all mutations delegate uncoerced input to storage');
