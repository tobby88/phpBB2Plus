...
foreach(array('ANONYMOUS'=>-1,'USER_AVATAR_NONE'=>0,'USER_AVATAR_UPLOAD'=>1,'USER_ACTIVATION_NONE'=>0,'ALLOW_VIEW'=>1,'CHECKBOX'=>3,'RADIO'=>2,'TEXTAREA'=>1,'TEXT_FIELD_MAXLENGTH'=>255,'TEXTAREA_MAXLENGTH'=>60000,
 ...
 'PROFILE_FIELDS_TABLE'=>'fixture_profile_fields','DISALLOW_TABLE'=>'fixture_disallow','WORDS_TABLE'=>'fixture_words',
...
$pp_tables=array('users','sessions','sessions_keys','groups','user_group','banlist','profile_fields','disallow','words','album','album_comment','ina_comment','ina_at_scores','shout','ina_highscore');
...
function pp_reset(){
 global $db,$userdata,$board_config,$public_avatar_scope,$pp_hook,$pp_after,$pp_write,$pp_fail,$pp_commit,$pp_queries,$pp_cookies,$phpbb_root_path;
 ...
 $board_config=array('require_activation'=>0,'avatar_path'=>'avatars','cookie_name'=>'fixture','cookie_path'=>'/','cookie_domain'=>'','cookie_secure'=>0);
...
function pp_run($scenario){
 ...
 $board_config['require_activation']=in_array($scenario,array('reactivate','reactivate-password'),true)?1:0;
 ...
 $profile_data=array(array('field_name'=>'user_custom_fixture','field_type'=>0,'text_field_maxlen'=>200));$HTTP_POST_VARS=array('user_custom_fixture'=>"Grüße ' 😀");
...
try {
 ...
 pp_sql('ALTER TABLE fixture_users ADD user_custom_fixture VARCHAR(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci DEFAULT NULL');pp_sql('SET SESSION innodb_lock_wait_timeout=1');$cases=0;
 foreach(array('edit','password','rename','avatar','reactivate','reactivate-password') as $scenario){
  pp_reset();$before=pp_snap();ats_check(pp_run($scenario)===true,'Valid self-service '.$scenario.' '.json_encode($pp_sql_error));$after=pp_snap();$writes=$pp_write;
 ...
 foreach(array('edit','password','reactivate','reactivate-password') as $scenario){
  pp_reset();pp_sql("UPDATE fixture_users SET user_newpasswd='reset-pending',user_actkey='pending-token' WHERE user_id=2");
 ...
 foreach(array('rename','reactivate') as $scenario){pp_reset();pp_sql($scenario==='rename'?"UPDATE fixture_users SET username='renamed' WHERE user_id=7":"UPDATE fixture_users SET user_email='new@example.invalid' WHERE user_id=7");$before=pp_snap();ats_check(pp_run($scenario)==='error'&&pp_snap()===$before&&!$pp_cookies,'Revalidate duplicate identity under writer lock');$cases++;}
