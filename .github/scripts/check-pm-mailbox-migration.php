<?php
namespace PmMailboxMigrationFixture;
...
eval('namespace PmMailboxMigrationFixture;'.substr($source,$begin,$end-$begin));
...
foreach (array('pm_delete_jobs','pm_delete_items') as $table)
{
...
foreach (array(array(),array('custom_pm_delete_jobs'),array('custom_pm_delete_jobs','custom_pm_delete_items')) as $pm_migration_existing)
{
    $operations=array(); eval('namespace PmMailboxMigrationFixture;'.$body);
    ...
    {
        pm_migration_check(strpos($sql,'CREATE TABLE custom_pm_delete')===0 && strpos($sql,'phpbb_')===false,'Custom prefix is honored');
...
pm_migration_check(strpos($create_statements['phpbb_pm_delete_jobs'],'UNIQUE KEY message_id')!==false,'One pending repair per message');
pm_migration_check(preg_match('/to_user_id int\(11\) NOT NULL/',$create_statements['phpbb_pm_delete_jobs'])===1,'Deleted PM participants remain signed');
pm_migration_check(strpos($create_statements['phpbb_pm_delete_jobs'],'user_password')===false,'No stored credential column in journal');
echo "Additive PM mailbox journal migration checks passed.\n";
$native_dsn=getenv('PHPBB_PM_MAILBOX_MIGRATION_TEST_DSN');
if($native_dsn!==false && $native_dsn!=='') {
    ...
    try {
        foreach(array(0,1,2) as $existing_count) {
            $connection->exec('DROP TABLE IF EXISTS custom_pm_delete_items,custom_pm_delete_jobs');
            $position=0;
            foreach($create_statements as $sql) { if($position++<$existing_count){$connection->exec(str_replace('phpbb_','custom_',$sql));} }
            if($existing_count>0){$connection->exec("INSERT INTO custom_pm_delete_jobs VALUES ('aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',42,8,'sentbox','prepared',8,-1,2,123,8,789)");}
            $operations=array();eval('namespace PmMailboxMigrationFixture;'.$body);
            ...
            pm_migration_check((int)$connection->query('SELECT COUNT(*) FROM custom_pm_delete_jobs')->fetchColumn()===($existing_count>0?1:0),'Existing recovery records preserved');
            if($existing_count>0){pm_migration_check((int)$connection->query('SELECT to_user_id FROM custom_pm_delete_jobs')->fetchColumn()===-1,'Signed sentinel preserved by actual DDL');}
            $operations=array();eval('namespace PmMailboxMigrationFixture;'.$body);pm_migration_check(!$operations,'Repeated actual native migration is empty');
            $indexes=$connection->query('SHOW INDEX FROM custom_pm_delete_jobs')->fetchAll(\PDO::FETCH_ASSOC);$message_unique=false;
        ...
        echo "Native missing/partial/repeated PM mailbox journal migration passed.\n";
    } finally {$connection->exec('DROP TABLE IF EXISTS custom_pm_delete_items,custom_pm_delete_jobs');}
}
