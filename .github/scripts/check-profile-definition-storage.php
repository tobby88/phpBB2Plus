...
class DefinitionConnectionFixture {
 ...
 function sql_query($sql,$tx=false){
  ...
  $r=$this->inner->sql_query($sql,$tx);if($sql==='COMMIT'&&$GLOBALS['pds_failure']==='ack'){return false;}if(strpos($sql,'ALTER TABLE `fixture_users`')===0&&$GLOBALS['pds_failure']==='ddl-ack'){return false;}return $r;
 }
}
...
$tables=array('users'=>'fixture_users','sessions'=>'fixture_sessions','jr_admin_users'=>'fixture_jr','banlist'=>'fixture_banlist','profile_fields'=>'fixture_profile_fields');
...
function pds_reset($actor='root'){
 ...
 $pds_failure='';$pds_hook=null;$pds_queries=array();pds_sql('START TRANSACTION');foreach($GLOBALS['tables'] as $t){pds_sql('DELETE FROM '.$t);}
...
try{
 ...
 echo 'Native definition edit: '.$cases." authorization, failure, concurrency, capacity and byte-preservation cases passed.\n";
